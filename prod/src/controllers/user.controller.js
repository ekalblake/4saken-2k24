...
export const failedUser = async (req,res) =>{
     ...
     return res.json(errors.response(HTTP_STATUS.UNAUTHORIZED,"No se pudo iniciar sesión, contáctate con un administrador."));
...
export const registerUser = async (req, res) => {
     ...
     try {
          ...
          const getUserInfo = await pool.query(`SELECT
                                                            ...
                                                            WHERE users_general.SteamID64 = ?`, [steamid])
          ...
          console.log(err);
          return res.json(errors.response(HTTP_STATUS.UNAUTHORIZED, "No se pudo obtener información del usuario, por favor comunicate con un administrador."));
     }
}
...
export const configUser = async (req, res) => {
    ...
    try{
        ...
        if (updateInfo.userid != useridSession)  return res.json(errors.response(HTTP_STATUS.UNAUTHORIZED, "This is not your account to update this configuration. Don't be silly."));
        ...
        console.log(err);
        res.json(errors.response(HTTP_STATUS.BAD_REQUEST, "There was an error while saving your configuration, contact an Admin please."));
    }
...
export const getUserMmr = async (req,res)=>{
     ...
     try{
          ...
          let getUserMmr = await pool.query(`SELECT
                                                       users_mmr.Rating,
                                                       users_mmr.Deviation,
                                                       users_mmr.GamesPlayed
                                                       FROM users_general 
          ...
          if(getUserMmr.length == 0) return res.json(errors.response(HTTP_STATUS.NOT_FOUND, "No se ha encontrado el jugador"));
