...
export const joinQueue = async (req, res) => {
	try {
		...
		const { region, isjoined } = req.body;

		const joinQueue = {
			region,
			isjoined,
		};
		//Revisa si el jugador esta en la queue
		...
		//Verifica que el jugador no esté en la cola
		...
		//Revisa si el jugador esta en la queue
		const checkMmr = await pool.query(
			`SELECT
                                                  users_mmr.Rating
                                                  FROM users_mmr
                                                  WHERE Pug_MMRID = ?`,
		...
		if (joinQueue.region === 2 && checkMmr[0].Rating <= 1500)
			return res
				.status(HTTP_STATUS.FORBIDDEN)
				.json(
					errors.response(
						HTTP_STATUS.FORBIDDEN,
						"No puedes ingresar a la cola de Adept por tu mmr actual. " +
							checkMmr[0].Rating,
					),
				);

		if (joinQueue.region === 3 && checkMmr[0].Rating <= 3000)
			return res
				.status(HTTP_STATUS.FORBIDDEN)
				.json(
					errors.response(
						HTTP_STATUS.FORBIDDEN,
						"No puedes ingresar a la cola de Veteran por el mmr Actual. " +
							checkMmr[0].Rating,
					),
				);

		//Verifica que no haya abandonado recientemente 300
		if (req.session.waitTime != 0 && joinTime - req.session.waitTime <= 5) {
			let waitTime = joinTime - req.session.waitTime;

			res.status(HTTP_STATUS.UNAUTHORIZED).json(
				errors.response(
					HTTP_STATUS.REQUEST_TIMEOUT,
					"Vuelve a unirte en " + waitTime + "/5 segundos.",
				),
			);
		} else {
			const checkLength = await pool.query(
				`SELECT queueid
                                                                 FROM 4saken.l4d2_queue
                                                                 WHERE region = ?`,
				[joinQueue.region],
			);
			...
			if (checkLength.length >= 8)
				return res
					.status(HTTP_STATUS.FORBIDDEN)
					.json(
						errors.response(HTTP_STATUS.UNAUTHORIZED, "Cola llena"),
					);

			let unixTimestamp = Math.floor(Date.now() / 1000);

			await pool.query(
				`INSERT INTO l4d2_queue
                                                (userid, region, joined_date)
                                            VALUES (?, ?, ?)`,
				[userid, joinQueue.region, unixTimestamp],
			);

			return res.json(
				errors.success(
					HTTP_STATUS.SUCCESSFUL,
					"Te has unido a la cola.",
				),
			);
		}
	} catch (err) {
		...
		return res
			...
			.json(
				errors.response(
					HTTP_STATUS.FORBIDDEN,
					"Hubo un error, por favor comunícate con un administrador.",
				),
			);
	}
};

export const dropQueue = async (req, res) => {
	try {
		const userid = req.session.userid;
		...
		let dropQueue = await pool.query(
			`DELETE FROM 4saken.l4d2_queue  WHERE userid = ?`,
			[userid],
		);
			...
			return res
				...
				.json(
					errors.response(
						...
						"No puedes salir de la cola si no estás en alguna.",
					),
				);
		...
		return res.json(
			errors.success(HTTP_STATUS.SUCCESSFUL, "Has salido de la cola."),
		);
	} catch (err) {
		return res
			.status(HTTP_STATUS.BAD_REQUEST)
...
export const dropQueueAdmin = async (req, res) => {
	try {
			...
			return res
				...
				.json(
					errors.response(
						...
						"No puedes kickear de la cola, actualiza la página.",
					),
				);
		...
		return res.json(
			errors.success(
				HTTP_STATUS.SUCCESSFUL,
				"Has kickeado a un jugador de la cola.",
			),
		);
	} catch (err) {
		console.log(err);
		return res
			...
			.json(
				errors.success(
					HTTP_STATUS.NOT_ACCEPTABLE,
					"Hubo un error, comunicate con un administrador.",
				),
			);
	}
};
	...
	try {
		...
		return res
			...
			.json(
				errors.response(
					HTTP_STATUS.BAD_REQUEST,
					"Ocurrió un error inesperado al listar las colas.",
				),
			);
	}
};
...
export const getMatchInfo = async (req, res) => {
	try {
		...
		return res
			.status(HTTP_STATUS.BAD_REQUEST)
			.json("No se pudo recopilar la información");
	}
};

/**
 * Esta funcion retorna el estado de la cola de Scout , Adept y Veteran
 */
