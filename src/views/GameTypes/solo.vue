     ...
     <v-container fluid color="transparent">
          ...
          <v-row>
               ...
               >
                    <queue-table 
                         :region="4" 
                         :queue-info="{ img: 'puggg.png', alt: 'Solo', type: 'Solo' }"
     ...
     setup(props) {
          ...
          const socketIo = () => {
               ...
               socketInstance.on('join:queue', (QueueUserJoined : IQueue) => {
                    ...
                    if (!queueUnranked.value?.getQueue() || queueUnranked.value?.getQueue().length === 8) return;

                    queueUnranked.value?.getQueue().push(new QueueItemModel(QueueUserJoined));
               ...
               socketInstance.on('drop:queue', (userid: number) => {
                    ...
                    let removeIndex = queueUnranked.value?.getQueue().map((item: QueueItemModel) => item.getUserId()).indexOf(userid);
               ...
               socketInstance.on('drop:players', () => {
                    ...
                    queueUnranked.value?.getQueue().splice(0, queueUnranked.value?.getQueue().length) 
          ...
          const getQueue = async () => {
               ...
               let findUserID = queueUnranked.value?.getQueue().map(( queue: QueueItemModel) => {
          ...
          //ADD QUEUE
          const addQueue = async (region: number, queueArray: QueueModel|null) => {
          ...
          watchEffect(() => {                     
               if (queueUnranked.value?.getQueue().length === 8) startQueue(queueUnranked.value);
          })
          ...
          onUnmounted(() => { 
               ...
               queueUnranked.value?.getQueue().forEach(async (q: QueueItemModel) => {
                    if (q.getUserId() == userInfo?.getUserId())
